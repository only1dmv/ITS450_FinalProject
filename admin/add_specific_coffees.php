...
?><h3>Add Specific Books</h3>

<form action="add_specific_coffees.php" method="post" accept-charset="utf-8">

	<fieldset><legend>Fill out the form to add specific book products to the site.</legend>
		
		<div class="field"><label for="category"><strong>Book Type</strong></label><br />
...
/*		$q = 'SELECT id, size FROM sizes ORDER BY id ASC';		
		...
		//$grinds = '<input type="text" name="title" value="<echo $grinds;>"
...
//		$caf_decaf = '<option value="caf">Caffeinated</option><option value="decaf">Decaffeinated</option>';
		...
		for ($i = 1; $i <= $count; $i++) {
			echo '<tr>
                        <td align="center"><input type="text" name="caf_decaf[' . $i . ']" $caf_decaf class="medium" /></td>
			<td align="center"><input type="text" name="ground_whole[' . $i . ']" $grinds class="medium" /></td>
